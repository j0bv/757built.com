    ...
    constructor(opts) {
            ...
            await new Promise((resolve, reject) => {
                ...
                content.forEach(async (file, i) => {
                    ...
                    if (typeof fstat !== "undefined") {
                        ...
                        if (e.category === "dir" && tcwd === settingsDir && file === "themes") e.type="757built-themesDir";
                        if (e.category === "dir" && tcwd === settingsDir && file === "keyboards") e.type = "757built-kblayoutsDir";
                    ...
                    if (e.category === "file" && tcwd === themesDir && file.endsWith(".json")) e.type = "757built-theme";
                    if (e.category === "file" && tcwd === keyboardsDir && file.endsWith(".json")) e.type = "757built-kblayout";
                    if (e.category === "file" && tcwd === settingsDir && file === "settings.json") e.type = "757built-settings";
                    if (e.category === "file" && tcwd === settingsDir && file === "shortcuts.json") e.type = "757built-shortcuts";
        ...
        this.render = async (originBlockList, isDiskView) => {
            ...
            blockList.forEach((e, blockIndex) => {
                ...
                if (e.type === "757built-theme") {
                    cmd = `window.themeChanger("${e.name.slice(0, -5)}")`;
                }
                if (e.type === "757built-kblayout") {
                    cmd = `window.remakeKeyboard("${e.name.slice(0, -5)}")`;
                }
                if (e.type === "757built-settings") {
                    cmd = `window.openSettings()`;
                }
                if (e.type === "757built-shortcuts") {
                ...
                switch(e.type) {
                    ...
                    case "usb":
                        ...
                        break;
                    case "757built-theme":
                        icon = this.757builtIcons.theme;
                        type = "757built-UI theme";
                        break;
                    case "757built-kblayout":
                        icon = this.757builtIcons.kblayout;
                        type = "757built-UI keyboard layout";
                        break;
                    case "757built-settings":
                    case "757built-shortcuts":
                        icon = this.757builtIcons.settings;
                        type = "757built-UI config file";
                        break;
                    case "system":
                        icon = this.757builtIcons.settings;
                        break;
                    case "757built-themesDir":
                        icon = this.757builtIcons.themesDir;
                        type = "757built-UI themes folder";
                        break;
                    case "757built-kblayoutsDir":
                        icon = this.757builtIcons.kblayoutsDir;
                        type = "757built-UI keyboards folder";
                        break;
                    default:
